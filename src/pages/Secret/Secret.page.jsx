...
function SecretPage() {
  ...
  useDebounce(
    () => {
      fetchVideos(`/search?part=snippet&maxResults=25&q=${value}&key=AIzaSyC8toCuN-oEin0T2SyAC4tIhUZ3xwasUk0`);
    },
    [value],
    300
  );
    ...
    <>
      <Container>
        ...
        {/*<Row>
            ...
            <Row xs={1} md={2} lg={3}>
              {data && data.slice(1).map((video) => (
                <Col key ={video.id.videoId}>
                  <NavLink to="/detail"  onClick={() => setVideoValues(video.snippet.title, video.snippet.description,  video.id.videoId)}>
                    <Card
                      style={{
                        width: '350px',
                        height: '350px',
